...
@section('body')
    <section class="minwork">
        <div class="container">
            <h2 class="title  text-center wow fadeInUp" data-wow-duration="1s">{{$branding->title}}</h2>
            <div class="row">

                @foreach($allbranding  as $onebrand)
                     <div class="col-sm-4 itwork wow fadeInUp" data-wow-duration="1s"><a class="item" href="{{url("portfoliobranding/show/$onebrand->id")}}">
                        <div class="photo">
                            <img src="{{asset("uploads/$onebrand->main_image")}}" alt="Smart target" title="Smart target">
                        </div>
                        <div class="content">
                            <div class="w-100">
                                <h4 class="title">{{$onebrand->title}}</h4>
                                <p class="text"> <?php echo $onebrand->s_description?> </p>
                            </div>
                        </div></a></div>
                @endforeach
            </div>
            {{$allbranding->links("web.paginator.pagination")}}
        </div>
        <ul class="color">
