...
@section('body')
    <section class="minwork">
        <div class="container">
            <h2 class="title  text-center wow fadeInUp" data-wow-duration="1s">{{$social->title}}</h2>
            <div class="row">



                @foreach($allsocial  as $onesocial)
                    <div class="col-sm-4 itwork wow fadeInUp" data-wow-duration="1s"><a class="item" href="#">
                            <div class="photo"> <img src="{{asset("uploads/$onesocial->main_image")}}" alt="Smart target" title="Smart target"></div>
                            <div class="content">
                                <div class="w-100">
                                    <h4 class="title">{{$onesocial->title}}</h4>
                                    <p class="text"> <?php echo $onesocial->s_description?> </p>
                                </div>
                            </div></a></div>
                @endforeach
            </div>
            {{$allsocial->links("web.paginator.pagination")}}
        </div>
        <ul class="color">
