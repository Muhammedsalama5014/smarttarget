...
@section('body')
    <section class="minwork">
        <div class="container">
            <h2 class="title  text-center wow fadeInUp" data-wow-duration="1s">{{$motion->title}}</h2>
            <div class="row">



                @foreach($allmotion  as $onemotion)
                    <div class="col-sm-4 itwork wow fadeInUp" data-wow-duration="1s"><a class="item" href="{{url("portfoliomotion/show/$onemotion->id")}}">
                            <div class="photo">
                                <img src="{{asset("uploads/$onemotion->video5")}}" alt="Smart target" title="Smart target">

                            </div>
                            <div class="content">
                                <div class="w-100">
                                    <h4 class="title">{{$onemotion->title}}</h4>
                                    <p class="text"> <?php echo $onemotion->company?> </p>
                                </div>
                            </div></a></div>
                @endforeach
            </div>
            {{$allmotion->links("web.paginator.pagination")}}
        </div>
        <ul class="color">
